...
export default function MobileNav() {
  ...
  return (
    ...
    <button aria-label={t('components.mobileNav.toggleMenu')} class="sm:hidden" ref={buttonRef} onClick={toggleMenu}>
      ...
      <div ref={menuRef}
        class="translate-x-full fixed left-0 top-0 h-full w-full transform opacity-[0.98] duration-300 ease-in-out bg-gray-950 z-20">
          ...
          <For each={NAVIGATION}>
            {({ href, title }) => (
              <div class="px-12 py-4">
                <Link href={href} class="text-2xl font-bold tracking-widest text-gray-100">
                  {t(title)}
                </Link>
              </div>
            )}
